...
# ----------------------------------------------------------------------
# Pure-Python DCF computation (mirrors the workbook's own formulas exactly,
# so the web page can show results without needing Excel/LibreOffice to
# evaluate formulas server-side).
# ----------------------------------------------------------------------

def _growth_series(fcf_current, growth_yr1_5, growth_yr6_10, growth_yr11_20, years):
    """List of projected FCF values for year offsets 1..years (matches the
    template's F29:O29 / F34:O34 chained-growth formulas)."""
    series = []
    prev = fcf_current
    for i in range(1, years + 1):
        if i <= 5:
            ...
            g = growth_yr6_10 or 0
        else:
            g = growth_yr11_20 or 0
        prev = prev * (1 + g)
        series.append(prev)
    return series


def compute_dcf(data, current_year, discount_rate, years):
    """Replicates, in Python, exactly what the template's formulas compute:
    the Yr1..Yr(years) FCF projection, PV of each year, PV of N-yr FCF,
    Intrinsic Value before cash/debt, less debt/share, plus cash/share,
    Final IV/share (statement & stock currency), and (Discount)/Premium.
    """
    fcf_current = data["fcf_current_value_m"] or 0
    fcf_series = _growth_series(
        fcf_current, data["growth_yr1_5"], data["growth_yr6_10"],
        data["growth_yr11_20"] if years == 20 else 0, years,
    )
    ...
    for i, fcf in enumerate(fcf_series, start=1):
        ...
        discounted_value = fcf * discount_factor
        pv_sum += discounted_value
        rows.append({
            "year": current_year + i,
            "fcf_m": fcf,
            "discount_factor": discount_factor,
            "discounted_value_m": discounted_value,
        })

    shares = data.get("shares_out_m")
    iv_before_cash_debt = (pv_sum / shares) if shares else None
    debt_per_share = (data["total_debt_m"] / shares) if shares and data.get("total_debt_m") is not None else None
    cash_per_share = (data["cash_sti_m"] / shares) if shares and data.get("cash_sti_m") is not None else None

    final_iv_stmt = None
    if iv_before_cash_debt is not None and debt_per_share is not None and cash_per_share is not None:
        final_iv_stmt = iv_before_cash_debt - debt_per_share + cash_per_share

    final_iv_stock = (final_iv_stmt * data["fx_rate"]) if final_iv_stmt is not None else None

    discount_premium = None
    if final_iv_stock not in (None, 0) and data.get("last_close") is not None:
        discount_premium = data["last_close"] / final_iv_stock - 1

    return {
        "years": years,
        "rows": rows,
        "pv_of_fcf_m": pv_sum,
        "iv_before_cash_debt": iv_before_cash_debt,
        "debt_per_share": debt_per_share,
        "cash_per_share": cash_per_share,
        "final_iv_per_share_stmt_ccy": final_iv_stmt,
        "final_iv_per_share_stock_ccy": final_iv_stock,
        "discount_premium": discount_premium,
    }


def analyze_ticker(ticker, year=None, manual_discount_rate=None):
    """Fetch data once and compute both the 10-year and 20-year DCF results.
    Used by the web portal to show results inline (no Excel file needed)."""
    year = year or default_year()
    data = fetch_stock_data(ticker, manual_last_close=None, manual_discount_rate=manual_discount_rate)
    result_10y = compute_dcf(data, year, data["discount_rate"], 10)
    result_20y = compute_dcf(data, year, data["discount_rate"], 20)
    return {"ticker": ticker, "year": year, "data": data, "result_10y": result_10y, "result_20y": result_20y}
